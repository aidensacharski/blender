...
namespace blender::nodes::node_fn_bitwise_operation_cc {
...
{
  const char *name;
  bool enum_label = RNA_enum_name(rna_enum_node_bitwise_operation_items, node->custom1, &name);
  if (!enum_label) {
    name = "Unknown";
  }
...
//  }
//
//  for (const EnumPropertyItem *item = rna_enum_node_bitwise_operation_items;
...
static const mf::MultiFunction *get_multi_function(const bNode &bnode)
{
  ...
  static auto xor_fn = mf::build::SI2_SO<int, int, int>(
      "Exclusive Or", [](int a, int b) { return a ^ b; }, exec_preset);
  static auto complement_fn = mf::build::SI1_SO<int, int>(
      "Complement", [](int a) { return ~a; }, exec_preset);
  static auto shift_left_arithmetic_fn = mf::build::SI2_SO<int, int, int>(
      "Shift Left", [](int a, int b) { return a << b; }, exec_preset);
  static auto shift_right_arithmetic_fn = mf::build::SI2_SO<int, int, int>(
      "Shift Right", [](int a, int b) { return a >> b; }, exec_preset);
  ...
  switch (bnode.custom1) {
    ...
    case NODE_BITWISE_AND:
      return &and_fn;
    case NODE_BITWISE_SHIFT_LEFT:
      return &shift_left_arithmetic_fn;
    case NODE_BITWISE_SHIFT_RIGHT:
      return &shift_right_arithmetic_fn;
    case NODE_BITWISE_XOR:
      return &xor_fn;
    case NODE_BITWISE_COMPLEMENT:
      return &complement_fn;
  }
...
static void node_rna(StructRNA *srna)
{
  RNA_def_node_enum(srna,
                    ...
                    "Operation",
                    "",
                    rna_enum_node_bitwise_operation_items,
